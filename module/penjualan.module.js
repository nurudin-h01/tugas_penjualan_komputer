...
class _penjualan {
	...
	updatePenjualan(data) {
		const sql = {
			query: `UPDATE s_penjualan SET id_customer = ?, id_produk = ?, jumlah_produk = ?, total_produk = ?, tanggal = ? WHERE id_penjualan = ?`,
			data: [
				data.id_customer,
				data.id_produk,
				data.jumlah_produk,
	...
	addPenjualan(data) {
		const sql = {
			query: `INSERT INTO s_penjualan (id_penjualan, id_customer, id_produk, jumlah_produk, total_produk, tanggal) VALUES (?, ?, ?, ?, ?, ?)`,
			data: [
				data.id_penjualan,
				data.id_customer,
				data.id_produk,
